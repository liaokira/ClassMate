...
exports.uploadNewImage = async (req, res) =>  {
    ...
    const {mimetype, buffer } = req.body;
    ...
    if(rows.length){
        ...
        if(path.includes('profile')){
            ...
            const {profile_rows} = await pool.query(profile_query, [image_id, id]);
            ...
            const {group_rows} = await pool.query(group_query, [image_id, id]);
...
exports.getImage = async (req, res) =>  {
    ...
    if(path.includes('profile')){
        ...
        const {profile_rows} = await pool.query(profile_query, [id]);
        if (profile_rows.length){
            image_id = rows[0].profile_pic_id;
        }
        else{
            res.status(404).send("No member profile found");
        }
        ...
        const {group_rows} = await pool.query(group_query, [id]);
        if (group_rows.length){
            image_id = rows[0].group_pic_id;
        }
        else{
            res.status(404).send("No member profile found");
        }
